...
// reportData holds the data injected into the HTML report template.
...
// htmlReportTemplate is the Go html/template version of the Python h-string.
// It reproduces the same HTML/CSS/SVG structure with proper html/template
...
<head>
    ...
    <title>GhostScan Security Scan Report</title>
    <style>
...
<body>
    <div class="container">
        <div class="logo">
            ...
            <!-- HackThacker Text -->
            <g transform="translate(150,100)">
                <text x="0" y="5" font-family="Arial, sans-serif" font-size="20" font-weight="bold" fill="url(#textGradient)" text-anchor="middle">HACKTHACKER</text>
            </g>
            </svg>
        </div>
        <h1 class="animated-text" data-text="HackThacker Security Scan Report">HackThacker Security Scan Report</h1>
        <div class="summary">
            <div class="summary-item">
                <span class="summary-label">Scan Type:</span>
                <span class="summary-value">{{.ScanType}}</span>
            </div>
            <div class="summary-item">
            ...
            <div class="summary-item">
                ...
                <span class="summary-value">{{.TotalScanned}}</span>
            </div>
            <div class="summary-item">
                <span class="summary-label">Time Taken:</span>
                <span class="summary-value">{{.TimeTaken}} seconds</span>
            </div>
        </div>
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                <div class="stat-value">{{.VulnRatePct}}</div>
        ...
        <div class="timeline">
            ...
            <div class="timeline-item right">
                <div class="timeline-content">
                    ...
                    <p>{{.TotalScanned}} URLs analyzed</p>
                </div>
            </div>
            <div class="timeline-item left">
                <div class="timeline-content">
                    <h3>Vulnerabilities Detected</h3>
                    <p>{{.TotalFound}} vulnerabilities found</p>
                </div>
            </div>
            <div class="timeline-item right">
                <div class="timeline-content">
                    <h3>Scan Completed</h3>
                    <p>Duration: {{.TimeTaken}} seconds</p>
                </div>
            </div>
        </div>
        <h2 class="animated-text" data-text="Vulnerable URLs">Vulnerable URLs</h2>
        <ul class="vulnerable-list">
            {{range .VulnerableURLs}}<li class="vulnerable-item"><a href="{{.}}" target="_blank" style="color: inherit; text-decoration: none;">{{.}}</a></li>{{end}}
        </ul>
    </div>
</body>
</html>`

// generateHTMLReport renders the HTML report into a string using html/template.
// Vulnerable URLs and dynamic values are properly escaped by the template engine.
func generateHTMLReport(scanType string, totalFound, totalScanned int, timeTaken int64, vulnerableURLs []string) (string, error) {
	progressPct := 0.0
	vulnRatePct := "0.00%"
	vulnRateBar := 0.0
	if totalScanned > 0 {
		progressPct = float64(totalFound) / float64(totalScanned) * 100
		vulnRateBar = float64(totalFound) / float64(totalScanned) * 100
	}

	// Format vulnerability rate as percentage string (e.g. "25.00%")
	vulnRatePct = fmt.Sprintf("%.2f%%", vulnRateBar)
	...
	data := reportData{
		...
		TimeTaken:      fmt.Sprintf("%d", timeTaken),
		VulnerableURLs: vulnerableURLs,
		ProgressPct:    progressPct,
		VulnRatePct:    vulnRatePct,
		VulnRateBar:    vulnRateBar,
	}
...
// saveHTMLReport writes the given HTML content to a file, appending .html if the
// filename doesn't already end with .html, and returns the absolute path.
func saveHTMLReport(htmlContent, filename string) (string, error) {
	...
	fmt.Printf("%s[i] Saving HTML report to %s%s\n", colorCyan, absolutePath, colorReset)

	if err := os.WriteFile(absolutePath, []byte(htmlContent), 0644); err != nil {
		fmt.Printf("%s[✗] Failed to save HTML report: %s%s\n", colorRed, err, colorReset)
		return "", err
	}

	fmt.Printf("%s[✓] HTML report saved as %s%s\n", colorGreen, absolutePath, colorReset)
...
func offerHTMLReport(scanType string, scanState *ScanState, startTime int64) {
	generateReport := promptYesNo("\n[?] Do you want to generate an HTML report? (y/n): ")
	if !generateReport {
		fmt.Println(colorRed + "\nExiting...")
		return
	}
	_, vulnURLs, found, scanned := scanState.Snapshot()
	timeTaken := elapsedSeconds(startTime)
	htmlContent, err := generateHTMLReport(scanType, found, scanned, timeTaken, vulnURLs)
	if err != nil {
		fmt.Printf("%s[✗] Error generating HTML report: %s%s\n", colorRed, err, colorReset)
		return
	}
	filename := promptString("[?] Enter the filename for the HTML report: ")
	saveHTMLReport(htmlContent, filename)
}

// offerHTMLReportDefault is the variant that prints a default filename if empty
// (used by the XSS scanner which suggests "xssreport.html").
func offerHTMLReportDefault(scanType string, scanState *ScanState, startTime int64) {
	generateReport := promptYesNo("\n[?] Do you want to generate an HTML report? (y/n): ")
	if !generateReport {
		fmt.Println(colorRed + "\nExiting...")
		return
	...
	filename := promptString("[?] Enter the filename for the HTML report or press Enter to use 'xssreport.html': ")
	if filename == "" {
		filename = "xssreport.html"
		fmt.Println(colorYellow + "[i] No filename provided. Using 'xssreport.html'.")
	}

	saveHTMLReport(htmlContent, filename)
}

// offerHTMLReportOR is the variant used by the Open Redirect scanner: only
// prompts if vulnerabilities were found, and uses a default filename of
// `open_redirect_report_<unix>.html` when none is supplied.
func offerHTMLReportOR(scanType string, scanState *ScanState, startTime int64) {
	vuln, vulnURLs, found, scanned := scanState.Snapshot()
	if !vuln {
		fmt.Println(colorYellow + "\n[i] No vulnerabilities found.")
		return
	}
	generateReport := promptYesNo("\n[?] Vulnerabilities found! Generate HTML report? (y/n): ")
	if !generateReport {
		return
	}
	timeTaken := elapsedSeconds(startTime)
	...
	filename := promptString("[?] Enter filename (or press Enter for default): ")
	if filename == "" {
		filename = fmt.Sprintf("open_redirect_report_%d.html", time.Now().Unix())
	}
	reportFile, err := saveHTMLReport(htmlContent, filename)
	if err == nil && reportFile != "" {
